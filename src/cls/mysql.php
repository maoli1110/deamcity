...
final class dbmysql {
    ...
    function getAll($sql, $resultType, $countsql='') {
        ...
        if ($this->perPage)
        {
            ...
            $this->rowCount = $this->result1($sql_count);
...
//        return mysqli_fetch_array($result, $type);
    ...
    {
        $this->lastqueryid = $this->execute($sql);
        $res = $this->fetch_next(MYSQLI_BOTH);
        return $res[$row];
    }

    public function result1($sql)
    {
        return $this->result($sql, 0);
    }

    public function get_total_num($db, $where)
    {
        $sql = "select count(*) from $db where $where ";
        return  $this->result1($sql);
    }
